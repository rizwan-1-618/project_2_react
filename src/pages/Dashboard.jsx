  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        ...
        <Link to="/transactions/new" className="glass-button" style={{ textDecoration: 'none' }}>+ New</Link>
      </div>
      
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '1.5rem', marginBottom: '2rem' }}>
        <div className="glass-card" style={{ padding: '1.5rem' }}>
          <p style={{ color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>Total Income</p>
          <h2 style={{ color: 'var(--success)', margin: 0 }}>{formatCurrency(totalIncome)}</h2>
        </div>
        <div className="glass-card" style={{ padding: '1.5rem' }}>
          <p style={{ color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>Total Expenses</p>
          <h2 style={{ color: 'var(--danger)', margin: 0 }}>{formatCurrency(totalExpenses)}</h2>
        </div>
        <div className="glass-card" style={{ padding: '1.5rem' }}>
          <p style={{ color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>Net Balance</p>
          <h2 style={{ color: totalIncome - totalExpenses >= 0 ? 'var(--text-primary)' : 'var(--danger)', margin: 0 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem' }}>
        ...
        <div className="glass-container" style={{ padding: '1.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            ...
            <Link to="/transactions" style={{ color: 'var(--primary-color)', textDecoration: 'none', fontSize: '0.9rem' }}>View All</Link>
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {recentTransactions.map(t => (
                <div key={t.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '0.5rem', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                  ...
                  <span style={{ color: t.type === 'income' ? 'var(--success)' : 'var(--text-primary)', fontWeight: '600', marginLeft: '1rem' }}>
